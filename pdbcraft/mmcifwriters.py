...
__doc__ = "Writers for mmCIF files."
...
class MMCIFWriter:
    ...
    # Set the character to be assigned to a value when the value is
    # unknown.
    UNKNOWN_CHARACTER = "?"
    ...
    def _get_format_strings_atom_site(self,
                                      struct):
        """Get the format strings to be used for data items in the
        '_atom_site' category.
        ...
        # category, the names of the non-atom-level items (plus the
        # 'id' item) mapped to the default width the corresponding
        # fields will have in the output mmCIF file.
        i2w = \
            {"group_PDB" : 6,
             "id" : 1,
        ...
        # Initialize a variable to store the current atom's unique ID.
        atom_id = 1
        ...
        for mod, mod_d in struct.atom_data.items():
            ...
            # If the string representing the current model is longer
            # than the longest one found so far
            if len(str(mod)) > i2w["pdbx_PDB_model_num"]:

                # Update the width that the corresponding field will
                # have.
            ...
            for ch, ch_d in mod_d["_items"].items():
                ...
                # If the string representing the current chain is
                # longer than the longest one found so far
                if len(str(ch)) > i2w["label_asym_id"]:

                    # Update the width that the 'label_asym_id' field
                    # will have.
                    i2w["label_asym_id"] = len(str(ch))

                    # Update the width that the 'auth_asym_id' field
                    # will have.
                ...
                for seg, seg_d in ch_d["_items"].items():
                    ...
                    for res, res_d in seg_d["_items"].items():
                        ...
                        # Get the residue's number, insertion code,
                        # and name.
                        ...
                        # residue's number is longer than the longest
                        # one found so far
                        if len(str(res_seq)) > i2w["label_seq_id"]:

                            # Update the width that the 'label_seq_id'
                            # field will have.
                            i2w["label_seq_id"] = len(str(res_seq))

                            # Update the width that the 'auth_seq_id'
                            # field will have.
                        ...
                        if len(str(i_code)) > i2w["pdbx_PDB_ins_code"]:
                            ...
                            # Update the width that the corresponding
                            # field will have.
                        ...
                        if len(str(res_name)) > i2w["label_comp_id"]:
                            ...
                            # Update the width that the 'label_comp_id'
                            # field will have.
                            i2w["label_comp_id"] = len(str(res_name))

                            # Update the width that the 'auth_comp_id'
                            # will have.
                        ...
                        for atom, atom_d in res_d["_items"].items():
                            ...
                            # Update the variable storing the atom's
                            # unique ID
                            atom_id += 1.
                            ...
                            in atom_d["_attributes"].items():
                                ...
                                # Otherwise
                                    ...
                                    # If the string representing the
                                    # the current attribute's value
                                    # is longer than the longest one
                                    # found so far.
        ...
        # Update the width of the item containing the atoms' unique
        # IDs.
        i2w["id"] = len(str(atom_id))

        #-------------------------------------------------------------#

        # Sort the data items in the dictionary according to the
        # order they usually have in mmCIF files.
        i2w = \
            {item : i2w[item] + 1 for item \
    ...
    def _get_format_strings_struct_conn(self,
        ...
        # data category, the names of the non-atom-level items (plus
        # the 'id' item) mapped to the default width they will have
        ...
        for mod in struct.conect_data:
            ...
            for atom1, atoms_bonded in struct.conect_data[mod].items():
                ...
                # If the length of the string representation of the
                # current chain is longer than the longest one found
                # so far.
                if len_atom1_ch > i2w["ptnr1_label_asym_id"]:

                    # Update the width that the 'ptnr1_label_asym_id'
                    # field will have.
                    i2w["ptnr1_label_asym_id"] = len_atom1_ch

                    # Update the width that the 'ptnr1_auth_asym_id'
                    # field will have.
                ...
                # If the length of the string representation of the
                # current residue's sequence number is longer than the
                # longest one found so far.    
                if len_atom1_res_seq > i2w["ptnr1_label_seq_id"]:

                    # Update the width that the 'ptnr1_label_seq_id'
                    # field will have.
                    i2w["ptnr1_label_seq_id"] = len_atom1_res_seq

                    # Update the width that the 'ptnr1_auth_seq_id'
                    # field will have.
                ...
                # If the length of the string representation of the
                # current residue's insertion code is longer than the
                # longest one found so far.
                if len_atom1_i_code > i2w["pdbx_ptnr1_PDB_ins_code"]:

                    # Update the width that the corresponding field
                    # will have.
                ...
                # If the length of the string representation of the
                # current residue's name is longer than the longest
                # one found so far.
                if len_atom1_res_name > i2w["ptnr1_label_comp_id"]:

                    # Update the width that the 'ptnr1_label_comp_id'
                    # field will have.
                    i2w["ptnr1_label_comp_id"] = len_atom1_res_name

                    # Update the width that the 'ptnr1_auth_comp_id'
                    # field will have.
                ...
                # If the length of the string representation of the
                # current atom's name is longer than the longest one
                # found so far.
                ...
                for atom2, bond_attrs in atoms_bonded.items():
                    ...
                    if len_atom2_ch > i2w["ptnr2_label_asym_id"]:
                        ...
                        # Update the width that the 
                        # 'ptnr2_label_asym_id' field will have.
                        i2w["ptnr2_label_asym_id"] = len_atom2_ch

                        # Update the width that the 
                        # 'ptnr2_auth_asym_id' field will have.
                    ...
                    if len_atom2_res_seq > i2w["ptnr2_label_seq_id"]:
                        ...
                        # Update the width that the 
                        # 'ptnr2_label_seq_id' field will have.
                        i2w["ptnr2_label_seq_id"] = len_atom2_res_seq

                        # Update the width that the 
                        # 'ptnr2_auth_seq_id' field will have.
                    ...
                    if len_atom2_res_name > i2w["ptnr2_label_comp_id"]:
                        ...
                        # Update the width that the 
                        # 'ptnr2_label_comp_id' field will have.
                        i2w["ptnr2_label_comp_id"] = len_atom2_res_name

                        # Update the width that the 
                        # 'ptnr2_auth_comp_id' field will have.
                    ...
                    # If the length of the string representation of
                    # the current atom's name is longer than the
                    # longest one found so far.
    ...
    def _get_format_strings(self,
                            struct):
        """Get the format strings to be used for each data item in the
        output mmCIF file.

        Format strings are returned for the following data categories:
        ...
        fmt_strings : ``dict``
            ...
            for the items' values in the output mmCIF file.
        """
        ...
        for category, i2w in item2width.items():
            ...
            # For each item's name and width of the corresponding
            # field in the output mmCIF file
        ...
        # Return the dictionary of format strings.
        return fmt_strings
    ...
    def _write_items(self,
        ...
        for item_ix, (item, fmt_str) in enumerate(fmt_strings.items()):
            ...
            # If an exception arises, it means the value is unknown,
            # so format strings for floating-point numbers and integers
            # will fail since the character used for unknown values is
            # a string.
            except ValueError:

                # Modify the format string so that it accepts strings.
                fmt_str = fmt_str[:-2] + "s}"
            ...
            # Write a whitespace character between each item.
            file_handle.write(" ")
    ...
    def _write_atom_type(self,
                         struct,
                         file_handle):
        """Write the ``Structure``'s unique atom types to the
        '_atom_type' category.
        ...
        # Get the unique atom type symbols per model.
        unique_type_symbols = \
        ...
        if unique_type_symbols:
            ...
            # Write out a comment character to end the category.
            file_handle.write("#\n")
    ...
    def _write_atom_site(self,
        ...
        # Set a counter for the atoms' unique IDs (they are continuous
        # in mmCIF files amd do not restart with every model).
        atom_id = 1
            ...
            for ch, ch_d in mod_d["_items"].items():
                ...
                for seg, seg_d in ch_d["_items"].items():
                    ...
                    for (res_seq, res_i_code, res_name), res_d \
                        ...
                        # Add the model's number to the dictionary
                        # of values to be used for the current record.
                        items["pdbx_PDB_model_num"] = mod

                        # Add the chain ID to the dictionary of values
                        # to be used for the current record.
                        items["label_asym_id"] = ch

                        # Add the chain ID to the dictionary of values
                        # to be used for the current record also for
                        # the 'auth_sym_id' item.
                        items["auth_asym_id"] = ch

                        # Add the residue's number to the dictionary of
                        # values to be used for the current record.
                        items["label_seq_id"] = res_seq

                        # Add the residue's number to the dictionary of
                        # values to be used for the current record also
                        # for the 'auth_seq_id' item.
                        items["auth_seq_id"] = res_seq

                        # Add the residue's insertion code to the
                        # dictionary of values to be used for the
                        # current record.
                        items["pdbx_PDB_ins_code"] = res_i_code

                        # Add the residue's name to the dictionary of
                        # values to be used for the current record.
                        items["label_comp_id"] = res_name

                        # Add the residue's name to the dictionary of
                        # values to be used for the current record also
                        # for the 'auth_comp_id' item.
                        items["auth_comp_id"] = res_name

                        # If we need to write a HETATM record
                        if res_d["_attributes"]["is_het"]:
                            
                            # Set the corresponding 'group_PDB' for the
                            # current record.
                        ...
                        for atom, atom_d in res_d["_items"].items():
                            ...
                            # Update the dictionary of values to be
                            # be used for the current record with the
                            # atom's attributes (we are guaranteed
                            # that all atoms' attributes are among the
                            # items, because of how the items'
                            ...
                            # corresponding to 'label_atom_id' using
                            # the value of 'label_atom_id'.
                            try:
                            ...
                            # attribute
                                ...
                                errstr = \
                                    "Atoms must have a " \
                                    "'label_atom_id' attribute to " \
                                    "write a valid mmCIF file."
    ...
    def _write_struct_conn(self,
        ...
        # Initialize an empty set to keep track of which bonds were
        # written out, so that we do not write them out twice.
        written = set()

        #-------------------------------------------------------------#

        # Initialize an empty list to store the sorted connectivity
        # records.
        sorted_conect = []
        ...
        for i, (mod, mod_data) in \
            ...
            # (= the atom the record refers to and the record itself,
            #  containing the atoms bonded to the first one and
            # information about the bond)
            for atom1, atoms_bonded in mod_data.items():

                # For each bonded atom and attributes associated with
                # the bond
                for atom2, bond_attrs in atoms_bonded.items():
                    ...
                    # Update the list storing the connectivity records.
            ...
            # Sort the list with the connectivity records by:
            #
            # - The bond's type.
            # - The bond's index.
            sorted_conect = \
            ...
            for (atom1, atom2), items in sorted_conect:
                ...
                # If the bond has not been written
                if (atom1, atom2) not in written:
                    ...
                    # Update the set of bonds that have already been
                    # written so that we do not write them twice.
    ...
    def write(self,
        ...
        with open(file, "w") as out:
            ...
            # Get the format strings to be used for the different
            # data categories in the mmCIF file.
            fmt_strings = \
