...
class PDBParser:
    ...
    def _get_atom_data(self,
        ...
        fix_residues_overflow : ``bool``, ``True``
            ...
            If ``True``, a warning will be raised and the
            numbering fixed.
        ...
        oldids2newids : ``dict``
            A dictionary mapping the atoms' old identifiers to
        ...
        # between atoms' serials and the atoms' unique identifiers
        # (made, for each model, of the chain ID, the segment ID,
        # the residue ID, and the atom name)
        serials2ids = {}

        # Initialize an empty dictionary to store the mappinf
        # between the original atom identifiers and the new ones
        # (it is especially useful to keep track of what was
        # what when we renumber residues because of
        # 'overflow' problems)
        oldids2newids = {}
        ...
        # Initialize a variable to keep track of which model
        # we were on at the previous ATOM/HETATM record to None
        prev_model = None

        # Initialize a variable to keep track of which chain ID
        # we were on at the previous ATOM/HETATM record to None
        prev_chain_id = None

        # Initialize a variable to keep track of the number of
        # the residue we were on at the previous ATOM/HETATM
        # record to None
        prev_res_seq = None

        # Initialize a variable to keep track of the original
        # number of the residue we were on at the previous
        # ATOM/HETATM record
        prev_res_seq_orig = None

        # Initialize a set to store the chains whose residues'
        # numbers needed fixing
        ...
        with open(file, "r") as f:
            ...
            for line_ix, line in enumerate(f, start = 1):
                ...
                # If the line contains an ATOM or HETATM record
                    ...
                    # If an alternative location is specified
                    # but it differs from the one we want
                    ...
                    # If we are dealing with the 'extended' PDB
                    # format, where the field usually dedicated
                    # to the insertion code is used to store the
                    # extra digit for structures with >9999 residues
                    if is_extended_pdb:

                        # Get the atom's residue's position in the
                        # sequence from the fields that usually 
                        # include the residue's position plus the
                        # field usually reserved for the
                        # insertion code
                        res_seq = int(line[22:27].split()[0])

                        # The insertion code is going to be
                        # an empty string
                        i_code = ""
                    ...
                    # Save the original sequence position
                    res_seq_orig = res_seq

                    # If we are in the same chain as before, but
                    # the current residue's number is lower than
                    # the number of the previous residue (= we are
                    # parsing a misformatted PDB file where numbering
                    # re-starts when we reach 10000 residues)
                    if chain_id == prev_chain_id and \
                        (prev_res_seq is not None \
                            and res_seq < prev_res_seq):

                        # If we are in a new residue
                        if prev_res_seq_orig != res_seq_orig:

                            # Initialize the error string to an
                            # empty string
                            errstr = ""
                            ...
                            if chain_id not in renumbered_chain_ids:
                                ...
                                # Tell the user about the overflow
                                errstr += \
                            ...
                            # Start a new residue with sequential
                            # numbering
                        ...
                        # Otherwise
                            ...
                            # We are in the same residue as the
                            # one before
                    ...
                    # If the residue identifier is not yet in the
                    # dictionary
                    ...
                    # Update the variable storing the model number
                    prev_model = model
                    
                    # Update the variable storing the ID of the chain
                    # we are on
                    ...
                    # Update the variable storing the original number
                    # of the residue we are on
                    ...
                    # Set the atom's new identifier, with the
                    # residue number used in the structure
                    atom_id_new = \
                        (chain_id,
                         seg_id,
                         (res_seq, i_code, res_name),
                    ...
                    if model not in serials2ids:
                        ...
                        # dictionary mapping the atoms' serials
                    ...
                    # Otherwise
                        ...
                        # mapping the atoms' serials to their
                        # original IDs
                    ...
                    if model not in oldids2newids:
                        ...
                        # Add an entry for the model in the
                        # dictionary mapping the original IDs to
                        # the ones used in the structure
                        oldids2newids[model] = \
                    ...
                    # Otherwise
                        ...
                        # Update the model's entry in the dictionary
                        # mapping the original IDs to the ones used
                        # in the structure
    ...
    def _get_conect_data(self,
        ...
        with open(file, "r") as f:
            ...
            # Log the fact that we are getting the connectivity
            # data
            debugstr = \
            ...
            for line_ix, line in enumerate(f, start = 1):
                ...
                if header == "CONECT":
                    ...
                    for atom_2_serial in atoms_bonded_serials:
                        ...
                        # If the atom is not in the record yet
                        else:
    ...
    def _get_ssbond_data(self,
        ...
        -------
        ssbond_data : ``dict``
            A dictionary containing the data from the SSBOND
            records.
        """

        # Initialize a dictionary to store the disulfide bonds
        ssbond_data = {}
        ...
        with open(file, "r") as f:
            ...
            # Log the fact that we are getting the disulfide
            # bonds' data
            debugstr = \
            ...
            for line_ix, line in enumerate(f, start = 1):
                ...
                # If the header indicates the section storing
                # disulfide bonds
                if header == "SSBOND":
                    ...
                    # Get the chain ID of the first cysteine
                    ...
                    # Get the chain ID of the second cysteine
                    ...
                    # Set the first SG atom's identifier (to
                    # add the bond later)
                    atom1_id = \
                        (cys1_chain_id, "",
                    ...
                    # Set the second SG atom's identifier (to
                    # add the bond later)
                    atom2_id = \
                        (cys2_chain_id, "",
                    ...
                    # Set the disulfide bond's identifier (to
                    # add the bond later)
        ...
        # Return the dictionary storing the disulfide bonds
        return ssbond_data
    ...
    def _get_link_data(self,
        ...
        # Initialize a dictionary to store the LINK data
        link_data = {}
        ...
        with open(file, "r") as f:
            ...
            debugstr = \
                "Now getting the 'link' bonds' data from " \
                "the LINK records..."
            logger.debug(debugstr)
            ...
            for line_ix, line in enumerate(f, start = 1):
                ...
                if header == "LINK  ":
                    ...
                    # Get the alternate location of the atom
                    ...
                    # Get the chain ID of the first residue
                    res1_chain_id = line[21]

                    # Get the sequence number of the first
                    # residue
                    ...
                    # Get the chain ID of the second residue
                    res2_chain_id = line[51]

                    # Get the sequence number of the second
                    # residue
                    ...
                    # If the first atom has an alternate location
                    ...
                    # If the second atom has an alternate location
                    # and it is not the one we want
                    ...
                    # Set the first atom's identifier (to
                    # add the bond later)
                    atom1_id = \
                        (res1_chain_id, "",
        ...
        # Return the dictionary storing the LINK data
        return link_data
    ...
    def _get_all_conect(self,
        ...
        for mod in conect_data:
            ...
            # Initialize a dictionary to store the current
            # unique index for disulfide and covalent bonds
            ...
            for disulf in ssbond_data:
                ...
                # Get the old IDs of the two atoms
                old_disulf_atom_1, old_disulf_atom_2 = disulf

                # Get the new ID of the first atom
                ...
                # If the first atom is not in the connectivity
                # data
                else:
            ...
            for link in link_data:
                ...
                if link_atom_1 in all_conect[mod]:
                    ...
                    # If the second atom is already in the
                ...
                if link_atom_2 in all_conect[mod]:
                    ...
                    # If the first atom is already in the
                    # bonded atoms of the second atom
            ...
            # For each 'first' atom in the dictionary of covalent
            # bonds
    ...
    def parse(self,
        ...
        infostr = \
            f"Structure '{file_name}' created from '{file}'."
        logger.info(infostr)
