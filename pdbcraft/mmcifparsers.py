...
class MMCIFParser:
    ...
    # Initialize the number to be used for unnamed models
    # in the structure built from the mmCIF file
    UNKNOWN_MODEL = 1

    # Initialize the character to be used for unnamed chains
    # in the structure built from the mmCIF file
    UNKNOWN_CHAIN_ID = ""

    # Initialize the character to be used for unnamed segments
    # in the structure built from the mmCIF file
    UNKNOWN_SEG_ID = ""

    # Initialize the number to be used for undefined residue
    # numbers in the structure built from the mmCIF file
    UNKNOWN_RES_SEQ = 1

    # Initialize the character to be used for undefined atom types
    # in the structure build from the mmCIF file
    UNKNOWN_TYPE_SYMBOL = ""
    ...
    def _format_token(self,
                      token):
        """Format string tokens parsed from the content of a mmCIF
        file.
        ...
        # If there is an underscore in the token (as it happens
        # for the '_struct.conn.ptnr1_symmetry' and 
        # _struct.conn.ptnr2_symmetry' values)
        if "_" in token:

            # Just return it as it is to avoid it being converted
            # to an integer
            return token
        ...
        # If the token could not be converted to an integer
            ...
            try:
                ...
                # If successful, return the token as a floating-
                # point number
                return float(token)
    ...
    def _get_tokens(self,
                    file):
        """Parse all lines of a mmCIF file and yield their
        constituent string tokens, one by one.
        ...
        with open(file, "r") as f:
            ...
            # Set a flag for whether we encountered a multi-line
            # quoted string and initialize it to False
            is_multiline = False

            # Set a variable to store the index of the last
            # encountered multi-line (for logging purposes)
            ...
            for line_ix, line in enumerate(f):
                ...
                # If the line is the beginning of a multi-line
                # quoted string
                elif line.startswith(\
                    _defaults.MMCIF_MULTILINE_CHARACTER):

                    # Strip the line of any trailing white spaces
                    # or new line characters
                    line = \
                        line.rstrip().lstrip(\
                            _defaults.MMCIF_MULTILINE_CHARACTER)

                    # If the line is otherwise empty (= end of
                    # a multi-line quoted string)
                    if not line:

                        # A multi-line has ended. Therefore,
                        # yield its content, separated by a 
                        # white space
                        yield " ".join(multiline_content)

                        # Reset the content of the multi-line
                        # section to be used for the next
                        # multi-line section encountered
                        multiline_content = []

                        # Set the multi-line starting index
                        # to None
                        multiline_start = None

                        # Set the flag specifying whether we are
                        # inside a multi-line section to False
                        is_multiline = False

                        # Go to the next line
                        continue

                    # If the line is not empty (= beginning
                    # of a multi-line quoted string)
                    else:
                        ...
                        if multiline_start is None:
                            ...
                            # Turn on the flag indicating that
                            # we are parsing a multi-line
                            # section
                            is_multiline = True

                            # Save the current index as the
                            # start of the multi-line
                            # section
                ...
                if is_multiline:
                    ...
                    # Add the content of the current line to
                    # the content of the multi-line section
            ...
            if is_multiline:
                ...
                errstr = \
                    ...
                    f"starting at line {multiline_start}."
    ...
    def _parse_as_dict(self,
        ...
        -------
        data : ``dict``
            The dictionary containing information parsed
            from the file.
        """
        ...
        # Initialize a variable storing the index of the current
        # line containing values parsed from a 'loop_' section
        current_loop_line_index = 0

        # Initialize a variable storing the index of the latest
        # item found in a 'loop_' section
        current_loop_item_index = 0

        # Initialize a list that will store the data items found
        # in a 'loop_' section
        current_loop_items = []
        ...
        for token in self._get_tokens(file = file):

            #---------------------------------------------------------#

            # If the token marks the start of a 'loop_' section
            # (the directives are case-insensitive, so we make sure
            # that we capture it even if it contains capital letters)
            if token.lower() == _defaults.MMCIF_LOOP_DIRECTIVE:

                # Set the 'loop_' flag to True
                is_loop = True

                # Reset the index keeping track of the index of
                # the last line parsed from the current 'loop_'
                # section
                current_loop_line_index = 0
                ...
                # Set the 'loop_' section's list of parsed items
                # to an empty list
                current_loop_items = []
            ...
            if is_loop:
                ...
                if is_data_item:
                    ...
                    # Get the name of the category the 'loop_'
                    # section belongs to and the data item
                    category, item = token.split(".")

                    # If there is no category with such a name
                    ...
                    # If we have already parsed lines containing
                    # values for the data items in this 'loop_'
                    # section (and, remember, the current token
                    # is a data item)
                    if current_loop_line_index > 0:

                        # The 'loop_' section has ended, and
                        # the current data item does not belong
                        # to it
                        is_loop = False

                    # Otherwise, we have encountered a new data
                    # item of the 'loop_' section
                    else:

                        # Add the item to the final dictionary
                        data[category][item] = []

                        # Save the current item to the list
                        # of items found for the 'loop_'
                        # section
                        current_loop_items.append(item)

                        # Update the index of the last item
                        # found in the 'loop_' section
                ...
                # values)
                    ...
                    # Find the item the value belongs to
                    item = \
                        current_loop_items[\
                            current_loop_line_index % \
                            current_loop_item_index]

                    # Append the current token to the values
                    # associated with the item
                    data[category][item].append(\
                        self._format_token(token = token))

                    # Update the value of the index keeping
                    # track of the number of lines containing
                    # values that we have parsed
            ...
            if current_item is None:
                ...
                if token.startswith("_"):
                    ...
                    # Get the name of the category and of the
                    # data item
                    category, item = token.split(".")

                    # If the category is not in the data yet
                    if category not in data:
                    ...
                    # Update the variable keeping track of the
                    # data item we are currently parsing
                    current_item = item
            ...
            # Otherwise (= the current token is a value)
                ...
                # Reset the variable keeping track of the
                # category we are currently parsing
    ...
    def _check_category(self,
                        ...
                        category_name,
                        strict):
        """Check whether the values reported for each data item
        in a given data category comply with the mmCIF format
        specifications.

        Parameters
        ----------
        data : ``dict``
            The data parsed from the mmCIF file.

        category_name : ``str``
            The name of the category to be checked.

        strict : ``bool``
            Whether we are running in 'strict' mode or not.
            
            Running in 'strict' mode means that any violation
            raises an error, while in non-strict mode most
            violations only trigger a warning.
        """

        # Inform the user we are checking the category
        infostr = \
            "Now checking the values in the " \
        ...
        for item_name in _defaults.MMCIF_CATEGORIES[category_name]:
            ...
            # Evaluate whether any of the values is of an invalid
            # type and it is not an empty string. We check whether
            # the values are empty strings and not '?' or '.'
            # because those have already been converted into
            ...
            if item_default is not None:
                ...
                # with the default, before proceeding with the
                # checks
                item_vals = \
                    [v if v != "" else item_default \
                     for v in item_vals]
            ...
            # If the item has an associated controlled vocabulary
                ...
                # an empty string. We check whether the values
                # are empty strings and not '?' or '.' because
                # those have already been converted into empty
                # strings while parsing the mmCIF file
                is_any_not_voc = \
                ...
                if is_any_not_voc:
                    ...
                    errstr = \
                        ...
                        "belong to the controlled vocabulary " \
                        "associated with the item " \
                        f"({item_voc_str}) or be one of the " \
            ...
            # If the item is a pointer to another item
                ...
                # Get the category and the item the current
                # item points to
                ...
                if pointed_category not in data:
                    ...
                    # Set the warning/error string
                    errstr = \
                        f"No category '{pointed_category}' found. " \
                        "This category should be present when the " \
                        f"item '{category_name}.{item_name}' is " \
            ...
            # If the controlled vocabulary is a list
                ...
                # The list contains the boundaries of the
                # numerical interval of values the field
                # can contain
                ...
                # of the boundaries
                is_any_outide = \
                    any((isinstance(v, str) \
                            and v != "") or \
                        (not isinstance(v, str) \
                            and not min_val <= v <= max_val) \
                        for v in item_vals)
    ...
    def _get_atom_data(self,
        ...
        only_alt_loc : ``str``, ``"A"``
            For atoms having multiple alternate locations,
            keep only those with the given alternate
            location ID.
        ...
        # Initialize an empty dictionary to store the data that
        # will contain non-atom-level attributes
        non_atom_attrs = {}

        # Create a copy of the category that will contain all
        # atom-level attributes by modifying this copy in place
        ...
        # Initialize an empty dictionary to storem for each
        # model the atoms' identifers corresponding to a specific
        # chain, residue, and atom (to correctly identify bonds
        # later on)
        models2ids = {}
        ...
        # If no category containing the models was found
            ...
            # Warn the user that no models' numbers were found
            warnstr = \
                "No models' numbers found (missing field " \
                f"'{item_models}' in section '{category_name}'). " \
                "All atoms will be considered belonging " \
                f"to one model, numbered {self.UNKNOWN_MODEL}."
            logger.warning(warnstr)
        ...
        if use_auth_chain_ids:
            ...
            # Otherwise
                ...
                # Warn the user that the automatically-assigned
                # IDs will be used
                warnstr = \
                    "Atomic coordinates: the user requested " \
                    "to use the chain IDs defined by the " \
                    "authors, but none were found. " \
                    "The automatically assigned IDs will be " \
                    "used instead (missing item " \
                    f"'{category_name}.auth_asym_id'). The values " \
                    "associated with the item " \
        ...
        if use_auth_res_seqs:
            ...
            # Otherwise
                ...
                warnstr = \
                    ...
                    "the authors, but none were found. " \
                    "The automatically assigned numbers will be " \
                    "used instead (missing item " \
        ...
        if use_auth_res_names:
            ...
            # Otherwise
                ...
                # names will be used
                warnstr = \
                    "Atomic coordinates: the user requested to " \
                    "use the residues' names defined by the " \
                    "authors, but none were found. " \
                    "The automatically assigned names will be " \
                    "used instead (missing item " \
        ...
        if use_auth_atom_names:
            ...
            # Otherwise
                ...
                # names will be used
                warnstr = \
                    "Atomic coordinates: the user requested to use " \
        ...
        if not item_types_symbols in category:
            ...
            # Warn the user that no symbols were found
            warnstr = \
                "No symbols representing the atoms' types found " \
                f"(missing item '{item_types_symbols}' " \
                f"in category '{category_name}'. " \
                "Each atom's type's symbol will be " \
        ...
        for atom_ix in range(len(non_atom_attrs["id"])):
            ...
            # Get the atom's segment ID
            seg_id = non_atom_attrs["seg_id"][atom_ix]

            #---------------------------------------------------------#

            # Get the original sequence number of the residue
            ...
            # If the chain identifier is not yet in the dictionary
            ...
            # If the segment identifier is not yet in the dictionary
            ...
            if not res_seq_orig:
                ...
                # the previous one, and the previous one
                # originally had an undefined sequence number, too
                if not current_res[3] and current_res[2] != res_name:

                    # Set the current residue's number it to
                    # the previous residues' number, plus 1
            ...
            # Otherwise
                ...
                # Set it to the original number
                res_seq = res_seq_orig

            #---------------------------------------------------------#
            
            # If the residue number is not in the dictionary yet
            if (res_seq, res_i_code, res_name) not in \
                atom_data[model]["_items"][chain_id]["_items"][\
                    seg_id]["_items"]:

                # If the original residue's number was undefined
                if not res_seq_orig:

                    # Warn the user
                    warnstr = \
                        f"Residue '{res_name}' in " \
                        f"chain '{chain_id}', model " \
                        f"{model}, has no sequence number. " \
                        f"The number {res_seq} was " \
                        f"assigned to it."
                    logger.warning(warnstr)

                # Add the residue to the data dictionary
                atom_data[model]["_items"][chain_id]["_items"][\
                    seg_id]["_items"][\
            ...
            for item, values in atom_attrs.items():
                ...
                if item == "label_alt_id":
                    ...
                    # If the alternate location is not the one
                    # we want to keep
                ...
                # Otherwise
                    ...
                    # Add the item and the corresponding
            ...
            # If we are now in another model with respect to that
            # of the last atom
            if model != current_model:

                # Reset the count for the atoms' serials
                serial = 1

            #---------------------------------------------------------#

            # If we are now in another chain with respect to that
            # of the last atom and we are in the same model
            ...
            # Add the data for the current atom to the data
            # dictionary
            ...
            # Get the identifier for the current atom to be used
            # in the atom table
            atom_id_table = \
                (chain_id,
            ...
            # If we do not have the model yet in the dictionary
            # mapping each model to the atoms' identifiers
            if model not in models2ids:

                # Create an entry for the model in the table
            ...
            # Update the atoms' serial numbers' count
            serial += 1

        #-------------------------------------------------------------#

        # Return the dictionary containing the parsed data
        # and the table that will be used to correctly
        # assign connectivity records
    ...
    def _get_conect_data(self,
        ...
        models2ids : ``dict``
            A dictionary mapping unique atoms' identifiers to
            the serial numbers the atoms have in the different
            models. 
        ...
        # Get the category containing the connectivity data
        category = data[category_name]
        ...
        if use_auth_chain_ids:
            ...
            # Otherwise
                ...
                # IDs will be used
                warnstr = \
                    "Connectivity data: the user requested " \
                    "to use the chain IDs defined by the " \
        ...
        if use_auth_res_seqs:
            ...
            # Otherwise
                ...
                # numbers will be used
                warnstr = \
                    "Connectivity data: the user requested to " \
                    "use the residues' numbers defined by the " \
                    "authors, but none were found (missing " \
        ...
        if use_auth_atom_names:
            ...
            # Otherwise
                ...
                warnstr = \
                    "Connectivity data: the user requested to " \
                    "use the atoms' names defined by the " \
                    "authors, but none were found (missing " \
                    f"item '{category_name}.ptnr1_auth_atom_id'). " \
                    "The values associated with the item  " \
            ...
            # Otherwise
                ...
                warnstr = \
                    ...
                    "use the atoms' names defined by the " \
                    "authors, but none were found (missing " \
                    f"item '{category_name}.ptnr2_auth_atom_id'). " \
                    "The values associated with the item  " \
        ...
        # If there are insertion codes specified for the first
        # atoms in the bonds, use them. Otherwise, set the
        # insertion codes to empty strings
        atoms1_icodes = \
            category["pdbx_ptnr1_PDB_ins_code"] \
            if "pdbx_ptnr1_PDB_ins_code" in category \
            else [""] * len(atoms1_names)

        # If there are insertion codes specified for the second
        # atoms in the bonds, use them. Otherwise, set the
        # insertion codes to empty strings
        atoms2_icodes = \
        ...
        for atom_ix, \
            ...
            # Build the identifier for the first atom
            ...
            # Build the identifer for the second atom
            # involved in the bond
            atom2_id = \
                (atom2_chain_id,
    ...
    def parse(self,
        ...
        # Get the 'atom_data' dictionary and the table that will
        # be used to correctly identify the bonds, if recorded
        # in the data
        ...
        # Get the 'conect_data' dictionary
        conect_data = \
        ...
        # Renumber the atoms since parts of different chains may
        # have been separated in the mmCIF file
